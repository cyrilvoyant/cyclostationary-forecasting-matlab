function predictions = predict_P_CLIPER_Cyclo(noisy_signal, n_delta_t, T)
    % Cyclostationary CLIPER Prediction
    % Parameters:
    ...
    for t = n_delta_t + 1:n_points
        ...
        % Calcul du bon lambda
        numerator = (mu_t_h^2 -mu_t^2)^2 + sigma_t^2 - rho_t_h * sigma_t * sigma_t_h;
        denominator =  (mu_t_h^2 -mu_t^2)^2 + sigma_t^2;

        if denominator ~= 0
            lambda = numerator / denominator;
        else
            lambda = 0; % Cas limite si le dénominateur est nul
        end
